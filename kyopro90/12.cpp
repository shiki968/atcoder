...
int main(){
    int h, w; cin >> h >> w;
    int Q; cin >> Q;
    int dp[h][w]; rep(i,h) rep(j,w) dp[i][j]=0;
    int dr[] = {0,1,0,-1};  // row
    int dc[] = {-1,0,1,0};  // column
    // if(r<0 || h<=r || c<0 || w<=c) break;
    //(y,x) = (r,c) = (h,w)
    rep(i, Q){
        int t; cin>>t;
        if(t==1){
            ...
            ra--; ca--; rb--; cb--;
            flg = false;
            if(dp[ra][ca]==1 && dp[rb][cb]==1){
                if(ra==rb && ca==cb) {flg = true; goto f;}
                int dpf[h][w]; rep(i,h) rep(j,w) dpf[i][j]=0;
                P now = P(ra,ca);
                ...
                while(q.size()){
                    ...
                    rep(j,4){
                        int nr = now.fi + dr[j];
                        int nc = now.se + dc[j];
                        if(nr==rb && nc==cb) {flg = true; goto f;} 
                        if(nr<0 || h<=nr || nc<0 || w<=nc || dp[nr][nc]==0 || dpf[nr][nc]==1) continue;
                        q.push(P(nr,nc));
                        dpf[nr][nc] = 1;
                    }
                }
