...
int main(){
    ...
    //  前選んだ文字の右にある文字ｃの中で最左のもの
    //  i文字目の右にある文字ｊの中で最も左にあるインデックス
    //   a  t  c  o  d  e  r  s.size()
    /*   0  1  2  3  4  5  6  7
    ---------------------------
    a    0  7  7  7  7  7  7  7
    c    2  2  2  7  7  7  7  7
    ...
    */
    int c[n+1][26]; 
    rep(i,n+1) rep(j,26) c[i][j]=0;
    rep(i,26) c[n][i] = n;

    // Q:長さがKであるSの部分列のうち、辞書順で最小であるものを出力
    ...
    rep(i,k){
        rep(j,26){
            int next = c[now][j];
            int maxPossibleLength = n-next+i;  // ????
