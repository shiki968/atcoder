...
int main(){
    ...
    string a = "atcoder";
    int an = 0;
    int dp[sz(s)+1][8]; rep(i,sz(s)+1) rep(j,8) dp[i][j]=0;
    
    // dp[pos][atcoderの何文字目まで] = 通り数
    // i文字目まで見たとき、j文字目まで決めたときの通り数。
	...
	rep(i, sz(s)) {
        rep(j,8){
            // 青矢印
			dp[i + 1][j] += dp[i][j];
            // 赤矢印（k文字目があったら採用 ＝＞ -1文字目まで決まった数を追加）
            rep(k, sz(a)) if (s[i] == a[k] && j == k) dp[i + 1][j + 1] += dp[i][j];
		}
		rep(j,8) dp[i + 1][j] %= MOD1;
	}
    out(dp[sz(s)][7])
